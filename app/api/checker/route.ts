...
// Skip files larger than 100 KB before fetching
const MAX_FILE_BYTES = 100 * 1024;
// Truncate fetched text content to keep tool results lean
const MAX_CONTENT_CHARS = 3000;

// Extensions where presence in the tree is sufficient — no need to read content.
// Fetching these wastes tokens without adding review signal.
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // ── 2. Fetch tree + README in parallel ────────────────────────────────
        ...
        // ── 3. Load prompt ────────────────────────────────────────────────────
        const systemPromptPath = path.join(process.cwd(), 'defs', 'prompts', 'gen', 'hardware-reviewer.md');
        const systemPrompt = fs.readFileSync(systemPromptPath, 'utf-8');
        send({ type: 'debug', message: `→ Prompt: ${systemPrompt.length} chars from ${path.basename(systemPromptPath)}` });

        // ── 4. Build tool definitions ─────────────────────────────────────────
        const tools: Anthropic.Tool[] = [
          {
            name: 'get_file',
            description: 'Fetch the text contents of a file from the GitHub repository. Use this for BOM CSVs, markdown files, and config files where you need to verify content quality. Do NOT use for source code (.c, .cpp, .py, .js, .kicad_pro, .kicad_sch, etc.), compiled binaries, or hardware files (.step, .gbr, .uf2) — their presence in the tree is sufficient.',
            input_schema: {
              type: 'object' as const,
              properties: {
                path: {
                  type: 'string',
                  description: 'Path to the file relative to the repository root (e.g. "firmware/main.c")',
                },
              },
              required: ['path'],
            },
          ...
          {
            name: 'get_image',
            description: 'Fetch and view an image. Use this for screenshots of 3D models, PCB renders, wiring diagrams, etc. Pass either a repo-relative path (e.g. "assets/photo.png") or a full URL for images embedded in the README via GitHub user-attachments or other external links.',
            input_schema: {
              type: 'object' as const,
              properties: {
                path: {
                  type: 'string',
                  description: 'Repo-relative path (e.g. "assets/photo.png") OR a full https:// URL for externally hosted images found in the README',
                },
              },
              required: ['path'],
            },
        ...
        // Exclude source/binary blobs from the tree display — directories still show
        // their presence, but listing every .c/.kicad_sch/etc. file bloats the context.
        const displayTree = tree.filter((e) => {
          ...
          // Show source/binary files at depth ≤1 (e.g. firmware/main.c) but
          // filter out deeply nested ones (e.g. firmware/Core/Src/main.c)
          return e.path.split('/').length <= 2;
        });
        const omittedCount = tree.filter(e => e.type === 'blob').length - displayTree.filter(e => e.type === 'blob').length;
        const treeText = formatTree(displayTree) +
          (omittedCount > 0 ? `\n(+ ${omittedCount} source/binary files omitted — their parent directories above indicate presence)` : '');
        send({ type: 'debug', message: `→ Tree: ${displayTree.length} entries shown, ${omittedCount} source/binary blobs omitted` });
...
Please review this repository against the submission criteria.`;
        ...
        for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
          ...
          // Retry on 529 overloaded with exponential backoff (up to 3 retries)
            ...
            try {
              if (block.name === 'get_image') {
                ...
                if (!isUrl && !isImagePath(filePath)) {
                  ...
                  const bytes = Math.round(data.length * 0.75); // base64 → bytes approx
                ...
                if (CONTENT_NOT_NEEDED_EXT.has(ext)) {
                  resultContent = `File exists in the repository tree. Reading content of ${ext} files is not needed — presence alone is sufficient for this review.`;
                  ...
                  resultContent = `File too large to display (${kb(treeEntry.size)}). The file exists — note its presence but do not try to read it.`;
